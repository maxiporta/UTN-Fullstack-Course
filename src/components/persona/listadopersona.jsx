...
import { urlroot, urlpersona, port } from '../../urls';
...
export default function ListadoPersona() {
    ...
    const[flag, setFlag] = useState([...startFlag(data.persona)]);
    ...
    let listaPersona = <p> no hay personas</p>;
    if(data.persona !==undefined)
    {
        listaPersona = data.persona.map((persona, index) => {
            let modificando = "";
            if(flag[index]===true)
            {
